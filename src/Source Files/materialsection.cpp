...
MaterialSection::MaterialSection(QWidget *parent) : QWidget(parent) {
    ...
    numMaterialsSpinBox->setValue(2);
        ...
        if (materialRowWidget) {
            ...
            if (materialRowLayout) {
                ...
                if (materialComboBox && nozzleComboBox && filamentComboBox) {
                    ...
                    // Extraer el valor numérico del filamento (en mm).
                    QString filamentText = filamentComboBox->currentText();
                    QRegularExpression filamentRegex(R"(\((\d+(\.\d+)?)\s*mm\))");  // Captura números con decimales dentro de "(x.xx mm)"
                    QRegularExpressionMatch filamentMatch = filamentRegex.match(filamentText);
                    if (filamentMatch.hasMatch()) {
                        config.filament = filamentMatch.captured(1);  // Guarda solo el valor numérico.
                        config.filamentSave = filamentComboBox->currentText();
                    }

                    // Extraer el valor numérico del nozzle (convertido a mm).
                    QString nozzleText = nozzleComboBox->currentText();
                    QRegularExpression nozzleRegex(R"((\d+(\.\d+)?)\s*µm)");  // Captura números con decimales antes de "µm"
                    QRegularExpressionMatch nozzleMatch = nozzleRegex.match(nozzleText);
                    if (nozzleMatch.hasMatch()) {
                        double nozzleValueMicrons = nozzleMatch.captured(1).toDouble();
                        config.nozzle = QString::number(nozzleValueMicrons / 1000.0, 'f', 3);
                        config.nozzleSave = nozzleComboBox->currentText();
                    } else {

                        qDebug() << "no coincide con:" << nozzleText;
                    }
...
void MaterialSection::updateMaterialNames(int numMaterials) {
    ...
    for (int i = 1; i <= numMaterials; ++i) {
        ...
        nozzleComboBox->setCurrentIndex(14);
