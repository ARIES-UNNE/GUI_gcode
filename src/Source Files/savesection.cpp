...
void SaveSection::readSectionValues() {
    ...
    while (!in.atEnd()) {
        ...
        if (line.contains("Section1 Values:")) {
            ...
            } else if (line.contains("Infill Value percentaje:")) {
            ...
            if (line.contains("Material Name:")) {
                QString materialName = "Custom";
                QString filament, nozzle;
                while (!in.atEnd()) {
                    line = in.readLine();
                    if (line.contains("FullFilament:")) {
                        filament = line.split(":").last().trimmed();
                    } else if (line.contains("FullNozzle:")) {
