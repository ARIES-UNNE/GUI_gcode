...
ShapeSection::ShapeSection(QWidget *parent) : QWidget(parent) {
    ...
    HeightComboBox->setPlaceholderText(tr("Select a Size"));
    HeightComboBox->addItem(tr("Number of Layers"));
    HeightComboBox->addItem(tr("Height"));

    // Crear y configurar las etiquetas descriptivas para los combo boxes.
    shapeLabel = new QLabel(tr("Shape Type"), this);
    sizeLabel = new QLabel(tr("Height Type"), this);

    // Crear y configurar el campo de entrada de texto para ingresar el tamaño de la forma.
    ShapeLineEdit = new QLineEdit(this);
    ShapeLineEdit->setPlaceholderText(tr("Enter size in mm"));
    ShapeLineEdit->setMaxLength(2);
    ShapeLineEdit->setText("20");
    HeightLineEdit = new QLineEdit(this);
    HeightLineEdit->setPlaceholderText(tr("Enter"));
    HeightLineEdit->setMaxLength(2);
    HeightLineEdit->setText("12");
    ...
    HeightComboBox->setMinimumWidth(uniformSize);
    HeightComboBox->setMaximumWidth(uniformSize);

    // Configuración inicial para los campos de entrada de texto (inicialmente ocultos).
    HeightLineEdit->setVisible(false);
    ShapeLineEdit->setVisible(false);
    HeightLineEdit->setMinimumWidth(uniformSize);
    HeightLineEdit->setMaximumWidth(uniformSize);
    ShapeLineEdit->setMinimumWidth(uniformSize);
    ShapeLineEdit->setMaximumWidth(uniformSize);

    // Crear un validador numérico para asegurar que solo se ingresen números en los campos de texto.
    QDoubleValidator* validator = new QDoubleValidator(this);
    ShapeLineEdit->setValidator(validator);  // Asignar el validador al primer campo de entrada.
    HeightLineEdit->setValidator(validator);  // Asignar el validador al segundo campo de entrada.
    ...
    horizontalLayout->addWidget(ShapeLineEdit);
    ...
    horizontalLayout2->addWidget(HeightComboBox);
    horizontalLayout2->addWidget(sizeUnitLabel);
    horizontalLayout2->addWidget(HeightLineEdit);
    ...
    connect(HeightComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &ShapeSection::handleShapeSelection);

    // Emitir señal de cambio de valor cuando cambia el texto del campo de entrada.
    connect(ShapeLineEdit, &QLineEdit::textChanged, [this]() {
        emit valueChanged();
    });

    connect(HeightLineEdit, &QLineEdit::textChanged, [this]() {
        emit valueChanged();
    });
    ...
    connect(HeightComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged), [this](int index) {
        emit valueChanged();
    });
...
void ShapeSection::handleShapeSelection(int index) {
    ...
    ShapeLineEdit->setVisible(showSizeLineEdit);
    ...
    bool showSizeLineEdit2 = (HeightComboBox->currentText() == tr("Number of Layers") || HeightComboBox->currentText() == tr("Height"));
    HeightLineEdit->setVisible(showSizeLineEdit2);

    // Actualizar el texto del placeholder según la forma seleccionada
    if (shapeComboBox->currentText() == tr("Square")) {
        ShapeLineEdit->setPlaceholderText(tr("Side"));
    } else if (shapeComboBox->currentText() == tr("Circle")) {
        ShapeLineEdit->setPlaceholderText(tr("Diameter"));
    }

    sizeUnitLabel->setVisible(showSizeLineEdit2 && HeightComboBox->currentIndex() >= 0);

    // Actualizar el texto de la etiqueta de unidad de tamaño según el tipo de tamaño seleccionado
    if (HeightComboBox->currentText() == tr("Height")) {
        sizeUnitLabel->setText(tr("mm:"));
    } else if (HeightComboBox->currentText() == tr("Number of Layers")) {
...
void ShapeSection::updateValues(int shapeIndex1, int shapeIndex2, int size1, int size2) {
    ...
    HeightComboBox->setCurrentIndex(shapeIndex2);
    ShapeLineEdit->setText(QString::number(size1));
    HeightLineEdit->setText(QString::number(size2));
}
...
void ShapeSection::retranslateUi() {
    ...
    HeightComboBox->setItemText(0, tr("Number of Layers"));
    HeightComboBox->setItemText(1, tr("Height"));
    shapeComboBox->setPlaceholderText(tr("Select a Shape"));
    HeightComboBox->setPlaceholderText(tr("Select a Size"));
    ShapeLineEdit->setPlaceholderText(tr("Enter size in mm"));
    HeightLineEdit->setPlaceholderText(tr("Enter"));
...
int ShapeSection::getSize1() const {
    return ShapeLineEdit->text().toInt();
}
...
int ShapeSection::getShapeIndex2() const {
    return HeightComboBox->currentIndex();
}
