...
Section4::Section4(QWidget *parent) : QWidget(parent) {
    ...
    section4Widget->setObjectName("section4Widget"); // Asignar un nombre de objeto a section4Widget
    numMaterialsSpinBox = new QSpinBox(this);
    numMaterialsSpinBox->setRange(0, 100);

    QLabel *plateSizeLabel4 = new QLabel("Number of Materials", this);
    plateSizeLabel4->setObjectName("plateSizeLabel4"); // Asignar un nombre de objeto a plateSizeLabel4
    ...
    QLabel *headerLabel4 = new QLabel("Additional Field", this);  // Nuevo encabezado para el nuevo campo
    ...
    headerLayout->addItem(new QSpacerItem(10, 0, QSizePolicy::Expanding, QSizePolicy::Minimum));
    headerLayout->addWidget(headerLabel4);  // Agregar el nuevo encabezado
    headerLayout->addItem(new QSpacerItem(10, 0, QSizePolicy::Expanding, QSizePolicy::Minimum));
    ...
    scrollArea->setObjectName("materialScrollArea"); // Asignar un nombre de objeto a scrollArea

    section4Layout->addWidget(scrollArea);

    section4Widget->setFixedSize(570, 535);

    QVBoxLayout *mainLayout = new QVBoxLayout(this);
    mainLayout->addWidget(section4Widget);
    ...
    for (int i = 0; i < materialNamesLayout->count(); ++i) {
        ...
        if (materialRowWidget) {
            ...
            if (materialRowLayout) {
                ...
                QLineEdit *additionalLineEdit = qobject_cast<QLineEdit*>(materialRowLayout->itemAt(3)->widget());  // Obtener el nuevo QLineEdit

                if (materialLabel && nozzleComboBox && filamentLineEdit && additionalLineEdit) {  // Incluir el nuevo campo
                    ...
                    config.algo = additionalLineEdit->text().toDouble();  // Guardar el valor del nuevo campo
...
void Section4::updateMaterialNames(int numMaterials) {
    ...
    for (int i = 1; i <= numMaterials; ++i) {
        ...
        materialRow->setObjectName("materialRowWidget"); // Asignar un nombre de objeto a materialRow
        ...
        QLineEdit *additionalLineEdit = new QLineEdit(this);  // Nuevo QLineEdit para el nuevo campo
        ...
        materialRowLayout->addWidget(additionalLineEdit);  // Agregar el nuevo campo al layout
        ...
        connect(additionalLineEdit, &QLineEdit::textChanged, this, &Section4::updateMaterialConfigs);  // Conectar la señal del nuevo campo
    }
